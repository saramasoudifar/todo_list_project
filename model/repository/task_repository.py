...
class TaskRepository():
    ...
    def save(self, task):
        self.connect()
        self.cursor.execute('INSERT INTO tasks (task_id, title, description, deadline, assigned_to,task.is_done) values (%s,%s,%s,%s,%s,%s)',
                       [task.task_id, task.title, task.description, task.deadline, task.assigned_to, task.is_done])
        self.disconnect(commit=True)

    def edit(self, task):
        self.connect()
        self.cursor.execute('update user set title = ?, description = ?, deadline = ?, assigned_to = ?,is_done =? where task_id = ?',
                       [task.task_id, task.title, task.description, task.deadline, task.assigned_to, task.is_done])
        self.disconnect(commit=True)
